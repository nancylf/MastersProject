...
# --- EXAMPLE --- DELETE THIS - make all lines gray, make one line colored.
# plot 3 months of AVG. AEROBIC C-CH4 data using replicate values
# normalized by gC

color_lines_shal <- summ_vals %>%
  filter(gas_sampled == "CH4" & headspace_treatment == "aerobic" & samp_int_day < 99 & sample_depth_range_m == "0.31-0.47")

color_lines_deep <- summ_vals %>%
  filter(gas_sampled == "CH4" & headspace_treatment == "aerobic" & samp_int_day < 99 & sample_depth_range_m == "18.52-18.67")

plt_avg_aer_ch4_gC_corr1 <- summ_vals %>%
  filter(gas_sampled == "CH4" & headspace_treatment == "aerobic" & samp_int_day < 99) %>%
  ggplot(aes(y = avg_accum_gas_ugC_gC, x = samp_int_day,
             color = sample_depth_range_m, linetype = factor(temp_treatment_C),
             group = interaction(sample_depth_range_m, temp_treatment_C))) +
  ...
  geom_line(mapping = NULL, data = color_lines_shal, color = "green") + 
  geom_line(mapping = NULL, data = color_lines_deep, color = "brown")
  # scale_color_manual(values = col_pal, limits = c("0.31-0.47", 
  #                                                   "0.6-0.76", 
  #                                                   "1.1-1.17", 
  #                                                   "3.05-3.21",
  ...
  #                                                   "18.52-18.67",
  #                                                   "19.83-19.98"))
# plt_avg_aer_ch4_gC_corr + gghighlight(sample_depth_lower >= 1852)

print(plt_avg_aer_ch4_gC_corr1)
